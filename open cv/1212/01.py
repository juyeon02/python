...
img2 = cv2.imread("images/puppy.jpg", cv2.IMREAD_GRAYSCALE)
...
gray_cat = cv2.imread("images/cat.jpg", cv2.IMREAD_GRAYSCALE)
color_cat = cv2.imread("images/cat2.jpg", cv2.IMREAD_COLOR)

### 2-1 shape 속성: 세로, 가로, (채널값)을 튜플형태로 반환
# 채널은 grayscale 사진일 경우는 값이 없음
print("gray cat image shape", gray_cat.shape) # (446, 446)
print("color cat image shape", color_cat.shape) # (512, 513, 3)
...
cv2.imwrite("output/gray_cat.png", gray_cat)